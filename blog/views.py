...
from .forms import DocumentForm, BlogForm
...
#                 don.message = message
        ...
        return render(request, 'details mockup.html', {'data1': data1, 'documents':documents, 'hashtag':hashtag})
...
def search(request):
    ...
    return render(request, 'search.html', {'results1':filtered1, 'ref': referer, 'err':err, 'hashtag':hashtag})
...
class AddHashtagview(CreateView):
    model = Hashtag
    template_name = 'add_hashtag.html'
    fields = '__all__'
    # hashtag = Hashtag.objects.get_or_create(name="AAAAAA")
    success_url = reverse_lazy('homepage')


class DelHashtagview(DeleteView):
    model = Hashtag
    template_name = 'del_hashtag.html'

    success_url = reverse_lazy('homepage')

    def get_context_data(self, **kwargs):
        context = super(DelHashtagview, self).get_context_data(**kwargs)
        hashtag = Hashtag.objects.all().order_by("name")
    ...
    def get_object(self, queryset=None):
        ...
        # Hashtag.objects.filter(pk=pk_).delete()
        return Hashtag.objects.filter(pk=pk_)
